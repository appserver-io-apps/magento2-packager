...
class FrontNameResolver implements \Magento\Framework\App\Area\FrontNameResolverInterface
{
    ...
    public function isHostBackend()
    {
        ...
        $scopeConfigKey = Store::XML_PATH_UNSECURE_BASE_URL;

        if (isset($_SERVER['HTTPS'])) {
            $scopeConfigKey = Store::XML_PATH_SECURE_BASE_URL;
        }

        if ($this->scopeConfig->getValue(self::XML_PATH_USE_CUSTOM_ADMIN_URL, ScopeInterface::SCOPE_STORE)) {
            $backendUrl = $this->scopeConfig->getValue(self::XML_PATH_CUSTOM_ADMIN_URL, ScopeInterface::SCOPE_STORE);
        } else {
            $backendUrl = $this->scopeConfig->getValue($scopeConfigKey, ScopeInterface::SCOPE_STORE);
        }
